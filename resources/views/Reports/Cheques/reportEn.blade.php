...
<div style="float:right;text-align: right;font-size:12px;background-color: #eee;padding:10px;direction:rtl; width:70%;">
    ...
    @else
    <span >{{ __('titles.cashed_to') }} : {{$Cheque->person_name}}</span><br>
    @endif
    <span >{{ __('titles.date') }} : {{date('d-m-Y', strtotime($Cheque->transaction_date))}}</span>
</div><br><br><br>
<div class="rep_name">
    @if ($Cheque->trans_type == 1)
    <span >{{ __('titles.recieve_cheques') }} : {{$Cheque->cheque_no}}</span>
    @else
    <span >{{ __('titles.cash_cheques') }} : {{$Cheque->cheque_no}}</span>
    @endif
